...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (email: string, senha: string): Promise<{ success: boolean; error?: string }> => {
    ...
    const { loginFuncionario } = await import('@/lib/supabase-actions')
    const supabaseResult = await loginFuncionario(email, senha)
    if (supabaseResult.success && supabaseResult.funcionario) {
      const f = supabaseResult.funcionario
      const newSession: AdminSession = {
        userId: f.email,
        nome: f.nome,
        cargo: f.setor === 'administracao' ? 'Administração' : 'Recepção',
        setor: f.setor,
      ...
      localStorage.setItem('filajusta_session', JSON.stringify(newSession))
      localStorage.setItem('filajusta_login_attempts', JSON.stringify({ attempts: 0, lockout: null }))
      return { success: true }
    }
    
    // Get registered admins
    const stored: AdminUser[] = JSON.parse(localStorage.getItem('filajusta_admins') || '[]')
    
    // Seed admin always available (matches Supabase funcionarios table)
    const seedAdmins = [
      {
        email: 'recepcao@vidaplena.com',
        passwordHash: '123456',
        nome: 'Recepção VidaPlena',
        cargo: 'Recepção',
      ...
      },
      {
        ...
        nome: 'Administração VidaPlena',
        cargo: 'Administração',
        setor: 'administracao' as const,
      },
    ]
    
    const allAdmins = [...seedAdmins, ...stored.map(u => ({ email: u.email, passwordHash: u.passwordHash, nome: u.nome, cargo: u.cargo, setor: u.setor }))]
    
    const user = allAdmins.find(
      a => a.email.toLowerCase() === email.toLowerCase().trim() && a.passwordHash === senha
    )
    ...
    if (!user) {
      ...
      if (newAttempts >= 5) {
        ...
        setLockoutUntil(lockout)
        localStorage.setItem('filajusta_login_attempts', JSON.stringify({ attempts: newAttempts, lockout }))
        return { success: false, error: 'Conta bloqueada por 15 minutos apos 5 tentativas.' }
      }
      
      localStorage.setItem('filajusta_login_attempts', JSON.stringify({ attempts: newAttempts, lockout: null }))
      return { success: false, error: 'Credenciais incorretas. Verifique e-mail e senha.' }
    }
