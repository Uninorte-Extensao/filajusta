...
export default function AdminPainelPage() {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const [medicosData, funcionariosData, especialidadesData] = await Promise.all([
        getAllMedicos(),
        getAllFuncionarios(),
        getEspecialidades(),
      ])
      ...
      if (funcionariosData.length === 0) {
        console.warn("[painel] Nenhum funcionário retornado pela API.")
      }
    } catch (err) {
  ...
  const handleMedicoSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const especialidade = especialidades.find(
        (e) => e.id === medicoForm.especialidade_id
      )
